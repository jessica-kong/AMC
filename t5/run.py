...
def train(args, model, tokenizer):
    ...
    train_dataset, train_examples = prepare_dataset(args=args, tokenizer=tokenizer, split="train",)
    ...
    valid_dataset, valid_examples = prepare_dataset(args=args, tokenizer=tokenizer, split="val",)
    ...
    train_bar = tqdm(range(args.max_train_steps), total=args.max_train_steps, ascii=True, )
    for epoch in range(args.num_epochs):
        ...
        if not early_stop.hit:
            logging.info(f"Early stopping counter: {early_stop.counter}/{early_stop.patience}")
...
def __eval(args, model, dataloader, tokenizer, examples, split, epoch=None):
    ...
    if split == 'test':
        ...
        df = pd.DataFrame(
            list(zip(inputs, input_pairs, all_golds, all_preds)),  # input_pairs,
            columns=["x1", 'x2,', "Gold", "Pred"],
        )
...
def main():
    ...
    # outputs and savings
    args.output_dir = os.path.join("..", "t5_outputs", args.run_name)  # root of outputs/savings

...
def run_eval(
    ...
    results.update({
        ...
        # "precision": precision(preds=preds, golds=golds),
        # "recall": recall(preds=preds, golds=golds),
        # "f1": f1(preds=preds, golds=golds)
    })
...
def run(args, accelerator: Accelerator):
    ...
    for epoch in range(args.num_epochs):
        ...
        # save the best model
        if early_stop.hit:
            save_best_dir = os.path.join(args.model_dir, f"best_acc")
            ...
            logger.info(f"The best acc model is saved to {save_best_dir}")
            ...
            #  test_table = pretty_results(test_results)
